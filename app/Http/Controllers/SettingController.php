...
class SettingController extends Controller
{
    ...
    public function livraisonSetting()
    {
        $lims_cities_data = City::all();
        return view('setting.livraison_setting', compact('lims_cities_data'));
    }
    ...
    public function livraisonStore(Request $request)
    {
        ...
        foreach ($data['id'] as $key => $city) {
            $lims_city_data = City::where('id', $data['id'][$key])->first();
            if ($data['livraison'][$key]) {
                $lims_city_data->livraison = $data['livraison'][$key];
            } else {
                $lims_city_data->livraison = '';
            }
            $lims_city_data->save();
        }
    ...
    public function generalSettingStore(Request $request)
    {
        ...
        $general_setting->referral = $data['referral'];
        $general_setting->api_id = $data['api_id'];
        $general_setting->api_key = $data['api_key'];
