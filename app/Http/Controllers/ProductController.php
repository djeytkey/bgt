...
class ProductController extends Controller
{
    ...
    public function productData(Request $request)
    {
        ...
        $lims_user_data = User::find(Auth::user()->id);
        if ($lims_user_data->is_vip) {
        ...
        if (!empty($products)) {
            foreach ($products as $key => $product) {
                ...
                $starting_date = date('Y-m-d', strtotime($product->starting_date));
                $last_date = date('Y-m-d', strtotime($product->last_date));

                if (($product->promotion_price) && ($todayDate >= $starting_date) && ($todayDate <= $last_date)) {
                    $nestedData['price'] = '<del>' . $product->price . '</del>&nbsp;&nbsp;' . $product->promotion_price;
                } else {
    ...
    public function updateProduct(Request $request)
    {
        ...
        if (!env('USER_VERIFIED')) {
            ...
            if ($request->prev_img) {
                $data['image'] = $lims_product_data->image . ',' . implode(",", $request->prev_img);
                // $lims_product_data->image = implode(",", $request->prev_img);
            } else {
                $data['image'] = "zummXD2dvAtI.png";
                $lims_product_data->image = "";
            }
            // else {
            ...
            if ($images) {
                ...
                if ($lims_product_data->image != 'zummXD2dvAtI.png') {
                    $data['image'] = $data['image'] . ',' . $lims_product_data->image . ',' . implode(",", $image_names);
                } else {
                    $data['image'] = $data['image'] . ',' . implode(",", $image_names);
                }
            } else {
                $data['image'] = $data['image'] . ',' . $lims_product_data->image;
            }
